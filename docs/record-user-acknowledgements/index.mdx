...
const notice = await httpsCallable(
  functions,
  'ext-firestore-record-user-acknowledgments-getNotice',
)({
  type: 'banner',
});
```

Note: if multiple instances of this extension are installed the callable function name might contain a unique identifier, e.g. `ext-firestore-record-user-acknowledgments-<id>-getNotice`.
...
const functions = getFunctions();
await httpsCallable(functions, 'ext-firestore-record-user-acknowledgments-acknowledgeNotice)({
...
```js
await httpsCallable(functions, 'ext-firestore-record-user-acknowledgments-unacknowledgeNotice)({
  noticeId: 'EA9QhZKKta9KXcckiasc',
  metadata: { reason: '...' },
});
```
...
const functions = getFunctions();
const acknowledgments  = await httpsCallable(functions, 'ext-firestore-record-user-acknowledgments-getAcknowledgments)();
```
