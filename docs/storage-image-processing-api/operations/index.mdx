...
<Image options={[{ operation: 'affine', matrix: [1, 0.3, 0.1, 0.7] }]} />
...
<Image options={[{ operation: 'blur', sigma: 10 }]} />
...
export const INPUT_OPTION = { operation: 'input', type: 'url', url: IMAGE_URL };

export const OUTPUT_OPTION = { operation: 'output', format: 'jpeg' };
...
export const Image = ({ options }) => (
  <div style={{ display: 'flex', alignItems: 'flex-start' }}>
    ...
    <img
      style={{ width: '50%' }}
      src={`${EXT_URL}?operations=${encodeURIComponent(
        JSON.stringify([INPUT_OPTION, ...options, OUTPUT_OPTION]),
      )}`}
    />
  </div>
);
